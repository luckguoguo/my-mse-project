...
<h1><@spring.messageText "orders.stock.list.label.title", "Stock Order Management" /></h1>
<form id="searchForm" action="${contextPath}/orders/stock/list/{status}/{startDate}to{endDate}/" method="GET">
</form>
<table>
	<tr>
		<td><@spring.messageText "orders.stock.list.label.status", "Status" /></td>
		<td>
			<select id="status">
			<#if statusList??>
	...
	<tr>
		<td><@spring.messageText "orders.stock.list.label.startDate", "Date From" /></td>
		<td>
			<input type="text" id="startDate" value="${startDate?string("yyyy-MM-dd")}" readonly="readonly" class="date-pick" />
		</td>
	</tr>
	<tr>
		<td><@spring.messageText "orders.stock.list.label.endDate", "Date To" /></td>
		<td>
	...
	<tr>
		<td></td>
		<td><input id="searchBtn" type="button" value="<@spring.messageText "orders.stock.list.label.submit", "Search" />" /></td>
	</tr>
</table>
...
<#if orders??>
	<table>
		<tr>
			<td><@spring.messageText "orders.stock.list.label.orderdate", "Order Date" /><td>
			<td><@spring.messageText "orders.stock.list.label.orderno", "Order No" /><td>
			<td><@spring.messageText "orders.stock.list.label.status", "Status" /><td>
			<td><@spring.messageText "orders.stock.list.label.num", "Num" /><td>
			<td><@spring.messageText "orders.stock.list.label.total", "Total Price" /><td>
			<td><@spring.messageText "orders.stock.list.label.operation", "Operation" /><td>
		</tr>
		...
		<tr>
			...
			<td><a href="${contextPath}/orders/stock/${order.id}"><@spring.messageText "orders.stock.list.label.detail", "See Detail" /></a></td>
		</tr>
		</#list>
	</table>
</#if>